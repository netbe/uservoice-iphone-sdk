...
- (void)loadView {
	...
	UIView *statusLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ORIENTATIONWIDTH, 8)];
	statusLine.backgroundColor = self.suggestion.statusColor;
	[contentView addSubview:statusLine];
	[statusLine release];
	
	UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 8, ORIENTATIONWIDTH, contentView.bounds.size.height - 8)];
	...
	scrollView.contentSize = CGSizeMake(ORIENTATIONWIDTH, body.bounds.size.height + 28);
